...
The statistical matching task starts by considering two independent survey samples from the same population of interest, each of which produces measures regarding specific questions (for example, living styles and wages). 

The basic assumption is that the sources share a block of variables (usually sociodemographic variables such as the age, sex, or social status), see Fig.1. 

![alt text](fig1.png)

**Fig 1. The starting point**

The basic assumption is that the number of individuals or units appearing in both samples (i.e., the overlap) is negligible. In this respect, the fundamental difference with respect to other methods such as _record linkage_ is that in the latter, we have identical units and we wish to find a correspondence between them; on the contrary, in in statistical matching, we _know_ the units are different, but we _wish_ to find similar ones.
...
We will typically obtain one of these results:
...
In a micro setting, we may want to fill one of the files. Then we say that:

* The file to be completed is the _recipient_ file, whereas
* The file that donates variables the other file is the _donor_. 

We may also want to fill variables in _both_ files: in this case no distinction is made between receptor and donor files, and we will say the files have a `symmetric` role.


**`receptor` and `donor` files in `micromatch`**

In a typical session with `micromatch`, we will start by defining a `receptor` and a `donor` file, or two `symmetric` files. These special _objects_ will contain not only the data frames with observations and variables (i.e. the microdata files themselves), but also some key information related to the statistical matching process. 

In the following we will illustrate this working mechanism by means of real examples.

The first example: matching `ecv` and `pra`
===========================================

Two microdata files have been included in `micromatch`, which correspond to two separate surveys conducted by Eustat (The Basque Statistical Office) during the 4th quarter of 2009: 
...
In the following steps, we will see how a synthetic file can be obtained starting from these two separate data frames. Each step of the matching task (Fig 2) is covered by a set of functions in `micromatch`.
...
`ecv` gives a bunch of living condition measures such as frequency of social relations, economic status, health problems, and so on. `pra` focuses on the labour market and produces a segmentation of the population in 5 categories. In this example we will try to obtain contingency tables crossing each of the `ecv` items with the labour market segmentation provieded by `pra`.
